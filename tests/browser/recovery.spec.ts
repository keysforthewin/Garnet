...
test('retries server and mirror failures, rejects stale restore, and deduplicates actions', async ({ page }) => {
  ...
  await page.locator('#new-doc').click(); await page.getByLabel('Document title').fill('Recovery check'); await page.locator('.prose').click(); await page.keyboard.type('Before outage.'); await expect(page.locator('#save-status')).toHaveText('Saved to server');
  ...
  execFileSync('docker', ['compose', '--profile', 'test', 'stop', 'test-app']);
  await page.locator('.prose').click(); await page.keyboard.press('Control+End'); await page.keyboard.type(' During outage.'); await expect(page.locator('#save-status')).toContainText('Saved locally');
  execFileSync('docker', ['compose', '--profile', 'test', 'up', '-d', 'test-app']);
  await expect(page.locator('#save-status')).toHaveText('Saved to server', { timeout: 45000 });
  ...
  try {
    ...
    await expect(page.locator('#save-status')).toHaveText('Saved to server');
...
test('injected storage quota failure keeps the editor usable and permits in-memory export', async ({ page }) => {
  await page.goto('/'); await page.getByLabel('Username', { exact: true }).fill('admin'); await page.getByLabel('Password', { exact: true }).fill('ed-test-password-2026'); await page.getByRole('button', { name: 'Sign in', exact: true }).click();
  await page.locator('#new-doc').click(); await page.getByLabel('Document title').fill('Quota recovery'); await page.locator('.prose').click(); await page.keyboard.type('This is locally editable.'); await expect(page.locator('#save-status')).toHaveText('Saved to server');
  ...
  await expect(page.locator('#save-status')).toContainText('Local storage failed'); await expect(page.locator('.prose')).toContainText('in-memory addition');
  const downloaded = page.waitForEvent('download'); await page.locator('#export-button').click(); const file = await downloaded; expect(await readFile((await file.path())!, 'utf8')).toContain('Preserve this in-memory addition.');
});
