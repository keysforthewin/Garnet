...
async function login(page: Page, username = 'admin', pass = password) {
  ...
  await expect(page.locator('#new-doc').or(page.getByRole('heading', { name: 'Make this account yours.' }))).toBeVisible();
}
...
test.describe.serial('shared editor', () => {
  ...
  test('create, persist, collaborate, restore cursor, edit offline and reconnect', async ({ browser, page }) => {
    ...
    await login(page); await page.locator('#new-doc').click();
    ...
    await page.locator('#new-doc').click(); await expect(page.getByLabel('Document title')).toHaveValue('Untitled');
  ...
  test('account restriction, settings, Markdown export, trash, restore, and theme', async ({ browser, page }) => {
    ...
    await writer.getByLabel('Current password', { exact: true }).fill(password); await writer.getByLabel('New password', { exact: true }).fill(`${password}-changed`); await writer.getByLabel('Confirm new password').fill(`${password}-changed`); await writer.getByRole('button', { name: 'Change password', exact: true }).click(); await expect(writer.locator('#new-doc')).toBeVisible();
    ...
    await page.locator('.doc-row').filter({ hasText: 'Shared field notes' }).click();
    const download = page.waitForEvent('download'); await page.locator('#export-button').click(); const exported = await download; expect(exported.suggestedFilename()).toMatch(/Shared-field-notes--.*\.md/); expect(await readFile((await exported.path())!, 'utf8')).toContain('Offline addition.');
    await page.locator('#more-button').click(); await page.getByRole('button', { name: 'Move to trash' }).click(); await expect(page.getByRole('heading', { name: 'Room to think.' })).toBeVisible();
